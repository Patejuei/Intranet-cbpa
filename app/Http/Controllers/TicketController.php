...
class TicketController extends Controller
{
    ...
    {
        $user = request()->user();
        if ($user->role !== 'admin' && $user->role !== 'capitan' && $user->role !== 'comandante' && $user->role !== 'secretaria_adquisiciones' && !in_array('tickets.view', $user->permissions ?? []) && !in_array('tickets.edit', $user->permissions ?? [])) {
            abort(403);
        }
    ...
    {
        $user = request()->user();
        if ($user->role !== 'admin' && $user->role !== 'capitan' && $user->role !== 'comandante' && $user->role !== 'secretaria_adquisiciones' && !in_array('tickets.edit', $user->permissions ?? [])) {
            abort(403);
        }
